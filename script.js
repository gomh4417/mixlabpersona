...
function generateQueryFromExcelData(data) {
  ...
  step2에 들어가는 행동변수는 step1에서 나온 것들로 연계해서 정보제공:
  {
      "step1": {
    ...
    current_persona_situation frustration goal motivation needs 는 최소 34자 이상으로 작성, scenario는 이 퍼소나가 서비스나 제품을 이용할 때 어떻게 사용할 건지 묘사해서 작성, place는 ${randomLocation} 으로 작성, 퍼소나의 갯수는 3개가 넘지 않는 선에서 알맞은 갯수로 작성
  `;
}
